...
/**
 * Base ConversionService implementation suitable for use in most environments. * Indirectly
 * implements ConverterRegistry as registration API through the * ConfigurableConversionService
 * interface.
 *
 * <p>适用于大多数环境的基本 ConversionService 实现。通过 ConfigurableConversionService 接口间接实现 ConverterRegistry
 * 作为注册 API。
 *
 * @author ChangZhen
 */
public class GenericConversionService implements ConversionService, ConverterRegistry {
    ...
    @Override
    public void addConverter(Converter<?, ?> convertor) {
        GenericConverter.ConvertiblePair typeInfo = getRequiredTypeInfo(convertor);
        ConverterAdapter converterAdapter = new ConverterAdapter(typeInfo, convertor);
